...
class AttackGraph(networkx.DiGraph):
    ...
    def path_risk(self, path):
        ...
        for node in path[1:]:
            if hasattr(node, 'impact'):
                node_risk = float (node.impact) * float(node.probability)
                #print (node.name, node.impact, node.probability, node_risk)
            else:
                node_risk = node.risk
    ...
    def percentage_severe_vulnerabilities(self):
        ...
        list_of_mediumvulns = []
        list_of_lowvulns = []
        total_vul = self.number_of_known_vulnerabilities()
        for node in self.hosts():
            for vul in node.lower_layer.all_vulns():
                if vul.risk>=7.0:
                    list_of_severevulns.append(vul)
                if vul.risk>=4.0 and vul.risk<7.0:
                    list_of_mediumvulns.append(vul)
                if vul.risk<4.0:
                    list_of_lowvulns.append(vul)

        return len(list_of_severevulns), len(list_of_mediumvulns), len(list_of_lowvulns)

    def number_of_vulnerable_hosts(self):
    ...
    def impact(self):
        ...
        return sum(self.path_impact(path) for path in self.all_paths)
...
def all_threat_node_risk(node):
    ...
    node_risk = impact_of_host(node) * float(node.probability)*threat_impact_sum
    return  node_risk

def sub_threat_node_risk(node,sub_stride):
    'sub_stride is an array of string belong to STRIDE'
    threat_impact_sum =0.0
